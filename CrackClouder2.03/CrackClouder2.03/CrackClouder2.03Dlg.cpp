...
void CCrackClouder203Dlg::OnBnClickedButton1()
{
	...
	*/
	if (-1 != GetFileAttributes(_T("C:\\Program Files\\iClass\\StudentClient"))) //如果文件夹存在， 最后的\\号有无都没关系
	{
		//文件存在 解锁按钮
		if (
			MessageBoxW(_T("检测完毕！你可以使用了。 后果自负哦~\n是否决定承担一切负担及责任并开始使用？"), _T("提示 | By Mactor | blog.mactor.xyz"), MB_ICONASTERISK | MB_YESNO)
			== IDYES)
		{
		...
		{
			if (
				MessageBoxW(_T("那就退出吧？"), _T("确认退出 | By Mactor |blog.mactor.xyz"), MB_ICONQUESTION | MB_YESNO)
				== IDYES
				)
			{
				CDialogEx::OnOK();
	...
	{
		MessageBoxW(_T("未检测到文件！"), _T("提示 | By Mactor |blog.mactor.xyz"), MB_ICONERROR | MB_OK);
	}
}
...
ShellExecute(0, NULL, _T("http://blog.mactor.xyz"), NULL, NULL, SW_NORMAL);
	MessageBoxW(_T("正在跳转 请稍后..欢迎访问！！\n PS:若出现乱码请使用Chrome浏览器打开"), _T("Mactor | blog.mactor.xyz"), MB_OK);
*/
}
...
{
	// 使用必读
	CString text = _T("欢迎使用！本程序目的是为了帮助各位在一定特殊需要下脱离教师的控制，请勿用于其他用途，解除控制后请尽快恢复控制！请勿用本程序用于其它用途！使用程序引起的一切后果（包括但不限于经济损失、文件等）均归使用者自行承担！\n作者：Mactor QQ2760502461  个人博客: blog.mactor.xyz\n本软件最终解释权归作者所有！\n\n点击“是”即代表你完全同意并承诺遵守以上内容! 若不同意或不承诺，请点击“否”退出并立刻将本程序从你的计算机上删除！");
